...
class BookControllerTest extends WebTestCase
{
    ...
    public function testUserCanCreateNewBook()
    {
        ...
        $newBookTitle = $crawler->filter('table > tbody > tr')->first()->filter('td:nth-child(3)')->text();
        $newBookAuthor = $crawler->filter('table > tbody > tr')->first()->filter('td:nth-child(4)')->text();
        $newBookDescription = $crawler->filter('table > tbody > tr')->first()->filter('td:nth-child(5)')->text();

        $this->assertSame('Mon super livre', $newBookTitle);
        $this->assertSame('Yoann Kergall', $newBookAuthor);
        $this->assertSame('Ma super description', $newBookDescription);
    }
